...
**Opção 2 — servidor HTTP local (recomendado para clipboard):** sirva a pasta `web/` com qualquer servidor de ficheiros estáticos (extensão **Live Server**, “Simple Browser” no editor, `npx serve web`, etc.). Na **raiz** do repositório, um exemplo é:

```bash
npx serve web
```

O terminal mostra um URL **Local** (ex.: `http://localhost:3000`) e, em muitos casos, um URL de **Network** na mesma LAN. Se a porta **3000** estiver ocupada, o `serve` escolhe outra (ex.: `63643`) e indica-o na mensagem. Abre o **Local** ou o **Network** no Chrome, Edge ou Firefox.
...
1. Com o servidor a correr, abre o URL indicado (ex.: `http://127.0.0.1:PORTA` ou o IP mostrado em **Network**).
2. Define **Comprimento** (ex.: `16`) e **Quantidade** (ex.: `1`).
3. Marca ou desmarca **Minúsculas**, **Maiúsculas**, **Dígitos** e **Símbolos** — por exemplo só **Símbolos** — e clica **Gerar**; confirma que o texto na caixa tracejada só contém símbolos permitidos.
4. Opcional: ativa **Garantir ao menos um caractere de cada conjunto marcado**, gera de novo e verifica que cada tipo marcado aparece na senha.
5. Clica **Copiar resultado** e confirma a mensagem verde; cola noutra aplicação para validar a área de transferência.
6. Opcional: introduz um comprimento fora de **8–64** ou combinações inválidas e confirma a **mensagem de erro** em vermelho acima do resultado.
